...
double ref_cte = 0; //Align with Line
double ref_epsi = 0;
double ref_v = 100;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    fg[0] = 0;  // init cost function to 0
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //The Upper and Lower limits of delta are set to -25 to 25 in rads

  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332*Lf; //remove Lf
